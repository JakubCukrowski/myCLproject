...
const Appointments = ({ currDay, weekDay, visitType }) => {
  ...
  const appointmentDate =
    currDay.toLocaleDateString("pl-PL").length < 10
  ...
  const updateVisit = async (time) => {
    ...
    const visitsQuery = query(
      usersCollection,
      where("date", "==", appointmentDate, where("time", "==", time))
    );
    ...
    await updateDoc(tempBlockedVisits, {
      tempBlockedVisits: arrayRemove({
        date: appointmentDate,
        time: time,
        type: visitType,
      }),
  ...
  const disableSavedVisits = (date, time) => {
    if (
      disabledTimes.some((data) => data.time === time && data.date === date)
    ) {
      return true;
    }
  };
    ...
    <>
        ...
        {weekDay === "Sobota" || weekDay === "Niedziela"
          ...
          : times.map((time) => {
              ...
              return (
                <button
                  key={time}
                  className={`visit-time-button ${
                    disableCloserVisits(currDay, time) ? "unavailable" : ""
                  }`}
                  disabled={
                    disableCloserVisits(currDay, time) ||
                    disableSavedVisits(appointmentDate, time) ||
                    tempBlockedButtons.some(
                      (btn) => btn.date === appointmentDate && btn.time === time
                    )
                  }
      ...
      {successMessage !== "" ? (
        <div className="confirmation-popup">
          <p>{successMessage}</p>
          <button className="ok-btn" onClick={handleSuccessMessage}>
            OK
          </button>
        </div>
      ) : null}
